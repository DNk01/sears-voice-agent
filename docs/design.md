...
**`audioop` deprecation** — The `audioop` module (used for PCM→mulaw conversion) is deprecated in Python 3.12 and removed in 3.13. A replacement using `numpy` + `scipy` or the `soundfile` library should be implemented before upgrading the runtime.
